...
class TravelerObject {
    ...
    function init()
    {
         ...
         * @since 1.1.7
         */
        add_action('ts_wc_show_order_item_meta_' . $this->post_type, [$this, '_wc_order_item_meta'], 10, 3);

        /**
         * @since 1.3.0
         */
        add_filter('rewrite_rules_array', [$this, 'ts_insert_rewrite_rules']);
        add_filter('query_vars', [$this, 'ts_insert_query_vars']);
        add_action('wp_loaded', [$this, 'ts_flush_rules']);
    ...
    public function ts_flush_rules() {
    ...
    public function ts_insert_rewrite_rules($rules)
    {
        /*$newrules = [];
        $slug = st()->get_option('slug_partner_page', 'page-user-setting');
    ...
    public function ts_insert_query_vars($vars) {
        array_push($vars, 'sc');
        return $vars;
    }
...
//        return ts_check_service_available($this->post_type);
    }
    ...
    function _class_init() {
        ...
        add_action('ts_single_breadcrumb', [$this, 'add_breadcrumb']);
        add_filter('wp_nav_menu_items', [$this, 'ts_custom_menu_mobile_item'], 10, 2);
    }
    ...
    function add_breadcrumb($sep) {
        $bc_show_location_url = trizen_get_option('bc_show_location_url', 'on');
        ...
        if (!empty($parents) and is_array($parents)) {
            for ($i = count($parents) - 1; $i >= 0; $i--) {
                ...
                if ($bc_show_location_url == 'on') {
                    ...
                    $page_search = ts_get_page_search_result($post_type);
                    ...
                    if (!empty($page_search) and get_post_type($page_search) == 'page') {
                        ...
                        $link = esc_url(add_query_arg([
                            'post_type'     => (!get_post_type()) ? 'ts_hotel' : get_post_type(),
                            's'             => '',
                            'location_id'   => $value,
                            'location_name' => get_the_title($value)
                        ], $link));
    ...
    function _top_ajax_search() {
        ...
        $search_header_orderby = trizen_get_option('search_header_orderby', 'none');
        $search_header_list    = trizen_get_option('search_header_list', 'post');
        $search_header_order   = trizen_get_option('search_header_order', 'ASC');
        $s = get('s');
        $arg = [
    ...
    function update_avg_rate($post_id) {
        ...
        // clear near by cache
        $range = trizen_get_option('hotel_nearby_range');
        $limit = 5;
        delete_transient('ts_items_nearby_' . $post_id . '_' . $range . '_' . $limit);
        delete_transient('ts_items_nearby_' . $post_id . '_20_' . $limit);
    }
    ...
    function properties_near_by($post_id, $lat, $lng, $range = 20) {
        ...
        FROM
            {$wpdb->prefix}ts_properties AS properties
        WHERE
            post_id = {$post_id}
        HAVING
            distance <= {$range}";
    ...
    function get_near_by($post_id = false, $range = null, $limit = 5) {
        ...
        if ($post_type == 'ts_rental') {
            $location_key = 'location_id';
        }
        $location_key = apply_filters('ts_' . $post_type . '_location_id_metakey', $location_key);
...
sin( radians( mt1.meta_value ) ) ) ) AS distance
        ...
        //set_transient('ts_items_nearby_' . $post_id . '_' . $range . '_' . $limit, $pageposts, 5 * HOUR_IN_SECONDS);
        return $pageposts;
    }

    function get_near_by_lat_lng($ts_location = false, $lat = false, $lng = false, $post_type = [], $range = 20, $limit = 5) {
        $map_lat = (float)$lat;
        $map_lng = (float)$lng;
        ...
        if (!empty($post_type) and is_array($post_type)) {
            ...
            AND $wpdb->posts.post_date < NOW()";
            $where = TravelHelper::edit_where_wpml($where);
            $where = TravelHelper::_ts_get_where_location($ts_location, $post_type, $where);
            $join = "";
            $join = TravelHelper::edit_join_wpml($join, $post_type);
...
sin( radians( mt1.meta_value ) ) ) ) AS distance
    ...
    function getOrderby() {
        $this->orderby = [
            'price_asc' => [
                'key'  => 'price_asc',
                'name' => esc_html__('Price ', 'trizen-helper') . ' (<i class="fa fa-long-arrow-down"></i>)'
            ],
            'price_desc' => [
                'key'  => 'price_desc',
                'name' => esc_html__('Price ', 'trizen-helper') . ' (<i class="fa fa-long-arrow-up"></i>)'
            ],
            'avg_rate' => [
                'key'  => 'avg_rate',
                'name' => esc_html__('Review', 'trizen-helper')
            ]
        ];
        return $this->orderby;
    }
    ...
    static function get_orgin_booking_id($item_id) {
        ...
        return apply_filters('ts_orgin_booking_item_id', $item_id);
    }
            ...
            if(array_key_exists('min_max_' . $post_type, self::$minMaxPrice)){
                ...
                switch ($post_type) {
                    ...
                    case "hotel_room":
                        ...
                        $sql = "SELECT
                                min(CAST( ts_hotel_room_price AS DECIMAL)) AS min,
                                max(CAST( ts_hotel_room_price AS DECIMAL)) AS max
                                FROM
                                (
                                    SELECT CASE
                                           ...
                                           END AS ts_hotel_room_price
                                    FROM
                                    {$wpdb->prefix}{$post_type}
                                    {$join}
                                    {$where}
                                )as ts_table_hotel_room";
                        $results = $wpdb->get_results($sql, OBJECT);
                        break;
                    case "ts_rental":
                        $where = "where 1=1 and ( {$wpdb->posts}.post_status = 'publish' )";
                        $where = TravelHelper::edit_where_wpml($where);
                        $sql = "SELECT
                                min(CAST( ts_rental_price AS DECIMAL)) AS min,
                                max(CAST( ts_rental_price AS DECIMAL)) AS max
                                FROM
                                (
                                    SELECT CASE
                    ...
                    case "ts_cars":
                        ...
                        $where = TravelHelper::edit_where_wpml($where);
                        $sql = "SELECT
                                min(CAST( ts_cars_price AS DECIMAL)) AS min,
                                max(CAST( ts_cars_price AS DECIMAL)) AS max
                                FROM
                                (
                                    SELECT CASE
    ...
    static function get_search_result_link($post_type = false) {
        ...
        return apply_filters('ts_' . $post_type . '_search_result_link', $url);
    }
    ...
    static function check_cancel_able($order_id) {
        ...
        $check_order = $wpdb->get_row("SELECT * FROM {$wpdb->prefix}ts_order_item_meta where user_id={$user_id} and  order_item_id=" . $order_id . " and `status`!='canceled' and `status`!='wc-canceled' LIMIT 0,1");
        if (!$check_order) {
            $return = FALSE;
        } else {
            ...
            if (get_post_meta($item_id, 'ts_allow_cancel', true) == 'on') {
                $days = (int)get_post_meta($item_id, 'ts_cancel_number_days', true);
                if ($days >= 0) {
                    if (dateDiff(date('Y-m-d'), date('Y-m-d', strtotime($check_order->check_in))) > $days) {
                        $return = true;
                    } else {
                        $return = false;
                    }
            ...
            $return = apply_filters('ts_check_cancel_able_' . $post_type, $return, $order_id, $item_id);
        }
        return $return = apply_filters('ts_check_cancel_able', $return, $order_id);
    }

    // from 1.1.9
    function _show_wc_cart_item_information_btn($cart_item_key = []) {
        //print balancetags("<br><p class ='btn btn-primary' data-toggle='collapse' data-target='#st_cart_item".md5(json_encode($cart_item_key))."'>".__("Details" , 'trizen-helper')."</p>");
        print balancetags('<br><span data-hide = "' . __("Less", 'trizen-helper') . ' <i class=&quot;fa fa-angle-up&quot;>" data-target= "#st_cart_item' . md5(json_encode($cart_item_key)) . '" data-toggle="collapse" class="_show_wc_cart_item_information_btn text-color booking-item-review-expand-more">' . __("More", 'trizen-helper') . ' <i class="fa fa-angle-down"></i></span>');
    }
