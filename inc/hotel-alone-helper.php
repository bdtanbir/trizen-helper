...
//		    $this->asset_url = ts_hotel_alone_load_assets_dir();
            add_action( 'wp_ajax_ajax_search_room_hotel_alone', [$this, 'ajax_search_room_hotel_alone'] );
            add_action( 'wp_ajax_nopriv_ajax_search_room_hotel_alone', [$this, 'ajax_search_room_hotel_alone'] );
	    }

        function ajax_search_room_hotel_alone(){
            if ( post( 'room_search' ) ) {
                if ( !wp_verify_nonce( post( 'room_search' ), 'room_search' ) ) {
                    $result = [
                        'status' => 0,
                        'data'   => "",
                    ];
                ...
                if ( empty( $hotel_id ) ) $hotel_id = request( 'hotel_id', 0 );
                $room_data = $this->search_room_by_id( $hotel_id );

                $style = request('helios-style', 'style-1');
                if(!empty($room_data)){
                    if($room_data['status']){
                        ...
                        if ( $query->have_posts() ) {
                            while ( $query->have_posts() ) {
                                $query->the_post();
                                $result[ 'data' ] .= 'room search result';
                            }
                            ...
                            $result = [
                                'status'         => 0,
                                'data'           => '',
                                'message'        => esc_html__( 'Our system does not find any rooms from your searching. You can change search feature now.', 'trizen-helper' ),
        ...
        function _alter_search_query_ajax($where) {
            ...
            if (request('start') and request('end')) {
                $check_in    = date('Y-m-d', strtotime(convertDateFormat(request('start'))));
                $check_out   = date('Y-m-d', strtotime(convertDateFormat(request('end'))));
...
/*                }
        ...
        function search_room_by_id( $hotel_id = false ) {
            ...
            if($check_in != '' && $check_out != '') {
                ...
                if(!$is_minimum_stay){
                    $result = [
                        'status'  => false,
                        'data'    => "",
                        'message' => sprintf(__('Please book at least %d day(s) in total.', 'trizen-helper'), $minimun_stay),
                    ];
                    return $result;
                }
            }
            $page = request( 'wpbooking_paged' );
            $arg  = [
                'post_type'      => 'hotel_room',
                'posts_per_page' => '4',
                'paged'          => $page,
            ...
            if($check_in != '' && $check_out != '') {
                ...
                $adult_num       = request('adults', 0);
                $child_num       = request('children', 0);
                $number_room     = request('room_number', 0);
