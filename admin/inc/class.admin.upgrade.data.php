...
//            add_action( 'wp_ajax_ts_get_data_location_nested', [$this, 'ts_get_data_location_nested'], 9999 );
        ...
        {
            if ( !in_array( $post_object->post_type, [ 'ts_hotel', 'st_rental', 'st_cars', 'st_tours', 'st_activity', 'hotel_room' ] ) ) {
                return $post_id;
            }
            global $pagenow;
...
//                $sql      = "SELECT count(ID) as total FROM {$wpdb->prefix}posts WHERE post_type in ('ts_hotel','st_rental','st_activity','st_tours','st_cars') AND post_status IN ('publish','private')";
