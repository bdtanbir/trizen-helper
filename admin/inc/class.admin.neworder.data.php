...
if ( !class_exists( 'NewOrderData' ) ) {
    class NewOrderData
    {
        public $table = 'ts_order_item_meta';
        public $ts_upgrade_order = 0;
        public $allow_version = false;

        public function __construct()
        {
            add_action( 'ts_save_order_item_meta', [ &$this, '_save_data' ], 10, 3 );
            add_action( 'ts_booking_change_status', [ &$this, '_ts_booking_change_status' ], 10, 3 );
            add_action( 'ts_traveler_do_upgrade_table', [ &$this, '_action_check_upgrade_order' ] );
            add_action( 'plugins_loaded', [ &$this, '_check_table_order' ], 10 );
            add_action( 'plugins_loaded', [ &$this, '_check_upgrade_order' ], 50 );
            add_action( 'admin_init', [ $this, 'upgrade_order_1_0' ] );
        }

        public function upgrade_order_1_0()
        {
            $updated = get_option( 'ts_upgrade_order_1_0', '' );
            if ( !$updated ) {
                global $wpdb;
                $sql = "UPDATE {$wpdb->prefix}ts_order_item_meta SET room_origin = room_id";

                $wpdb->query( $sql );
                update_option( 'ts_upgrade_order_1_0', 'updated' );
            }
        }

        public function _action_check_upgrade_order()
        {
            $this->ts_upgrade_order = 1;
        ...
        {
            $complete = get_option( 'ts_upgrade_order' );
            if ( !$complete || $complete == 0 || $this->ts_upgrade_order == 1 || $this->allow_version ) {
        ...
        public function _save_data( $data = [], $order_item_id = false, $type = 'normal_booking' )
        {
            if ( is_array( $data ) && count( $data ) ) {
                ...
                $post_type_check = get_post_type( $data[ 'ts_booking_id' ] );
                ...
                $ts_booking_post_type = $data[ 'ts_booking_post_type' ];
                $ts_booking_id        = $data[ 'ts_booking_id' ];
                $duration             = isset( $data[ 'duration' ] ) ? $data[ 'duration' ] : null;
                $adult_number         = isset( $data[ 'adult_number' ] ) ? $data[ 'adult_number' ] : 0;
                if ( $post_type_check == 'ts_flight' ) {
                ...
                $room_origin     = isset( $data[ 'room_id' ] ) ? post_origin( $data[ 'room_id' ], 'hotel_room' ) : null;
                ...
                $g_post          = get_post( $data[ 'ts_booking_id' ] );
                $partner_id      = $g_post->post_author;
                if ( !empty( $data[ 'ts_booking_post_type' ] ) && $data[ 'ts_booking_post_type' ] == 'ts_flight' ) {
                ...
                global $sitepress;
                if ( $sitepress ) {
                    $post_type = get_post_type( $ts_booking_id );
                ...
                $value = [
                    ...
                    'ts_booking_post_type' => $ts_booking_post_type,
                    'ts_booking_id'        => $ts_booking_id,
                    'user_id'              => $user_id,
        ...
        public function _ts_booking_change_status( $status, $order_id, $booking_type )
        {
        ...
        public function _duplicateOrder()
        {
            ...
            if ( is_array( $list_normal ) && count( $list_normal ) ) {
                foreach ( $list_normal as $key => $val ) {
                    ...
                    $ts_booking_post_type = isset( $val[ 'ts_booking_post_type' ] ) ? $val[ 'ts_booking_post_type' ] : '';
                    $ts_booking_id        = isset( $val[ 'ts_booking_id' ] ) ? $val[ 'ts_booking_id' ] : '';
            ...
            if ( is_array( $list_woo ) && count( $list_woo ) ) {
                foreach ( $list_woo as $key => $val ) {
                    $order_item_id        = $key;
                    $type                 = 'woocommerce';
                    $check_in             = isset( $val[ '_ts_check_in' ] ) ? date( 'm/d/Y', strtotime( $val[ '_ts_check_in' ] ) ) : '';
                    $check_out            = isset( $val[ '_ts_check_out' ] ) ? date( 'm/d/Y', strtotime( $val[ '_ts_check_out' ] ) ) : '';
                    $ts_booking_post_type = isset( $val[ '_ts_ts_booking_post_type' ] ) ? $val[ '_ts_ts_booking_post_type' ] : '';
                    $ts_booking_id        = isset( $val[ '_ts_ts_booking_id' ] ) ? $val[ '_ts_ts_booking_id' ] : '';
                    $duration             = isset( $val[ '_ts_duration' ] ) ? $val[ '_ts_duration' ] : '';
                    $adult_number         = isset( $val[ '_ts_adult_number' ] ) ? $val[ '_ts_adult_number' ] : 0;
                    $child_number         = isset( $val[ '_ts_child_number' ] ) ? $val[ '_ts_child_number' ] : 0;
                    $infant_number        = isset( $val[ '_ts_infant_number' ] ) ? $val[ '_ts_infant_number' ] : 0;
                    $discount             = isset( $val[ '_ts_discount' ] ) ? $val[ '_ts_discount' ] : '';
                    $room_id              = isset( $val[ '_ts_room_id' ] ) ? $val[ '_ts_room_id' ] : '';
                    $room_num_search      = isset( $val[ '_ts_room_num_search' ] ) ? $val[ '_ts_room_num_search' ] : '';
                    $check_in_timestamp   = isset( $val[ '_ts_check_in_timestamp' ] ) ? $val[ '_ts_check_in_timestamp' ] : strtotime( $check_in );
                    $check_out_timestamp  = isset( $val[ '_ts_check_out_timestamp' ] ) ? $val[ '_ts_check_out_timestamp' ] : strtotime( $check_out );
                    $status               = isset( $val[ 'order_id' ] ) ? get_post_status( $val[ 'order_id' ] ) : 'trash';
                    $wc_order_id          = isset( $val[ 'order_id' ] ) ? $val[ 'order_id' ] : '';
                    $user_id              = isset( $val[ '_ts_user_id' ] ) ? $val[ '_ts_user_id' ] : 1;
                    $g_post               = get_post( $ts_booking_id );
                    $partner_id           = $g_post ? $g_post->post_author : '';
                    $commission           = isset( $val[ 'ts_commission' ] ) ? $val[ 'ts_commission' ] : 0;
                    if ( $type == 'normal_booking' ) {
                        $total_order       = get_post_meta( $wc_order_id, 'total_price', true );
                        $booking_fee_price = get_post_meta( $wc_order_id, 'booking_fee_price', true );
                        if ( !empty( $booking_fee_price ) ) {
            ...
            update_option( 'ts_upgrade_order', 1 );
        }
        ...
        public function _getOldData(){
			...
			AND post_type = 'ts_order'
			AND (
			meta_key = 'ts_booking_post_type'
			or meta_key = 'ts_booking_id'
        ...
        public function _getOldDataWoo() {
			...
			AND(
			{$wpdb->prefix}woocommerce_order_itemmeta.meta_key = '_ts_adult_number'
			or {$wpdb->prefix}woocommerce_order_itemmeta.meta_key = '_ts_child_number'
			or {$wpdb->prefix}woocommerce_order_itemmeta.meta_key = '_ts_infant_number'
			or {$wpdb->prefix}woocommerce_order_itemmeta.meta_key = '_ts_discount'
			or {$wpdb->prefix}woocommerce_order_itemmeta.meta_key = '_ts_duration'
			or {$wpdb->prefix}woocommerce_order_itemmeta.meta_key = '_ts_check_in'
			or {$wpdb->prefix}woocommerce_order_itemmeta.meta_key = '_ts_check_out'
			or {$wpdb->prefix}woocommerce_order_itemmeta.meta_key = '_ts_ts_booking_post_type'
			or {$wpdb->prefix}woocommerce_order_itemmeta.meta_key = '_ts_ts_booking_id'
			or {$wpdb->prefix}woocommerce_order_itemmeta.meta_key = '_ts_user_id'
			or {$wpdb->prefix}woocommerce_order_itemmeta.meta_key = '_ts_check_in_timestamp'
			or {$wpdb->prefix}woocommerce_order_itemmeta.meta_key = '_ts_check_out_timestamp'
			or {$wpdb->prefix}woocommerce_order_itemmeta.meta_key = '_ts_room_num_search'
			or {$wpdb->prefix}woocommerce_order_itemmeta.meta_key = '_ts_room_id'
			or {$wpdb->prefix}woocommerce_order_itemmeta.meta_key = '_ts_commission'
			)
